...
import { setLaunchType } from '@/redux/reducers/launchReducer';
...
const LaunchComp = () => {
    ...
    const { launches, filteredLaunches, startDate, endDate, launchType, isModalOpen, selectedLaunch } = useSelector((state) => state.launch);
    ...
    if (session) {
        return (
            <div className="p-8 w-full h-full">
                <div className=''>
                    ...
                    <div className='flex justify-around'>
                        ...
                        <div className='flex flex-row justify-center items-center gap-8'>
                            <div className='w-80 h-64 p-2 flex flex-col justify-center items-baseline text-white border-2 border-white rounded-sm'>
                                <div className=''>
                                    ...
                                    <input
                                        ...
                                        className="px-2 py-1 rounded-md"
                                    /> <br /><br />
                                    <button onClick={handleDateFilter} className="px-4 py-2 text-xl hover:border-black hover:bg-white hover:text-black transition slide-in-from-top-0 border-2 border-white text-white">
                                        Filter
                                    </button>
                                </div>
                            ...
                            <LaunchPieChart totalLaunches={totalLaunches} successfulLaunches={successfulLaunches} failedLaunches={failedLaunches} />
                                ...
                                <div className='flex max-h-96 overflow-auto'>
                                    <table className="table-auto">
                                        ...
                                        {filteredLaunches.map((launch, index) => (
                                            <tr className='border-2 border-white' key={launch.id} onClick={() => toggleModal(launch)}>
                ...
                {
                    isModalOpen && (
                        <LaunchModal isModalOpen={isModalOpen} selectedLaunch={selectedLaunch} toggleModal={toggleModal} />
                    )
                }
            </div >
        );
